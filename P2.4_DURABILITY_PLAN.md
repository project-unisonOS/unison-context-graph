...
## Overview (Status: Implemented in service runtime)
...
- Write-Ahead Log (WAL) for durability **(enabled via `DurabilityManager`)**
- TTL-based automatic cleanup **(background TTL job + `/durability/run_ttl`)**
- PII scrubbing for privacy **(background scrub job + `/durability/run_pii`)**
- Crash recovery on startup **(checksum and WAL replay)**
